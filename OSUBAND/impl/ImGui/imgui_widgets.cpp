...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec4 target_col = (held && hovered) ? style.Colors[ImGuiCol_ButtonActive] : hovered ? style.Colors[ImGuiCol_ButtonHovered] : style.Colors[ImGuiCol_Button];

    ImGuiStorage* storage = window->DC.StateStorage;
    float speed = 10.0f * g.IO.DeltaTime;

    ImVec4 current_col;
    current_col.x = ImLerp(storage->GetFloat(id + 0, target_col.x), target_col.x, speed);
    current_col.y = ImLerp(storage->GetFloat(id + 1, target_col.y), target_col.y, speed);
    current_col.z = ImLerp(storage->GetFloat(id + 2, target_col.z), target_col.z, speed);
    current_col.w = ImLerp(storage->GetFloat(id + 3, target_col.w), target_col.w, speed);

    storage->SetFloat(id + 0, current_col.x);
    storage->SetFloat(id + 1, current_col.y);
    storage->SetFloat(id + 2, current_col.z);
    storage->SetFloat(id + 3, current_col.w);

    const ImU32 col = ColorConvertFloat4ToU32(current_col);
    RenderNavCursor(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);

    ImU32 outline_col = IM_COL32(17, 16, 17, 255);
    window->DrawList->AddRect(bb.Min, bb.Max, outline_col, style.FrameRounding, 0, 1.0f);

    if (held && hovered)
    {
        ImVec4 click_col = ImVec4(current_col.x * 0.8f, current_col.y * 0.8f, current_col.z * 0.8f, current_col.w);
        ImU32 overlay = ColorConvertFloat4ToU32(click_col);
        window->DrawList->AddRectFilled(bb.Min, bb.Max, overlay, style.FrameRounding);
    }
